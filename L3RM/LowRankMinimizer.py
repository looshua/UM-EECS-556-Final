...
class LowRankMinParams:
    ...
    step:           int     =   2                   # number of pixels between block centers
...
class LowRankMinimizer:
        ...
        for i in range(block_size):
            for j in range(block_size):
                for k in range(self.C):
                    X[i*block_size+j*self.C+k,:] = im_in[i:-block_size+i,
                                                         j:-block_size+j,
                                                         k].flatten()
        return X.T
    ...
    def _block_matching(self,
        ...
        for i in range(self.num_blocks):
            ...
            l2_error = np.linalg.norm(error,2,axis=2)/self.block_elems
    ...
    def _low_rank_SSC(self,
        ...
        for i in range(self.L):
            ...
            block_means = np.tile(np.mean(blocks,2),(n_blocks,1))
    ...
    def _block_SSC(self,
        ...
        sig_i = np.max(np.pow(sigma,2)/self.block_elems - self.sig_w**2, 0)
        tau = self.params.c1*self.sig_w**2 / (np.sqrt(sig_i) + np.finfo().eps)
    ...
    def _unpatchify(self,
        ...
        for i in range(self.params.block_size):
            for j in range(self.params.block_size):
                for k in range(self.C):
                    lin_index = (i*self.M)+(j*self.C)+k
                    im_out[i:self.n-i,j:self.m-j,k] = \
                        np.reshape(Y[lin_index,:].T,(self.N,self.M))
                    im_weights[i:self.n-i,j:self.m-j,k] = \
